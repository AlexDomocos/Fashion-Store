...
public class CategoryModel {
	...
	public Integer nextPK() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(ID) FROM F_Category");
	...
	public CategoryBean findByName(String name) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM F_Category WHERE NAME=?");
	...
	public CategoryBean findByPK(long pk) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM F_category WHERE ID=?");
		CategoryBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	...
	public long add(CategoryBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("INSERT INTO F_Category VALUES(?,?,?,?,?,?,?,?)");
			pstmt.setInt(1, pk);
	...
	public void delete(CategoryBean bean) throws ApplicationException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn.prepareStatement("DELETE FROM F_category WHERE ID=?");
	...
	public void update(CategoryBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE F_Category SET NAME=?,DESCRIPTION=?,image=?,CREATEDBY=?,MODIFIEDBY=?,CREATEDDATETIME=?,MODIFIEDDATETIME=? WHERE ID=?");
	 ...
	 public List search(CategoryBean bean) throws ApplicationException {
	    ...
	    public List search(CategoryBean bean, int pageNo, int pageSize)
	        ...
	        StringBuffer sql = new StringBuffer("SELECT * FROM F_category WHERE 1=1");
	        if (bean != null) {
	            if (bean.getId() > 0) {
	    ...
	    public List list(int pageNo, int pageSize) throws ApplicationException {
	        ...
	        ArrayList list = new ArrayList();
	        StringBuffer sql = new StringBuffer("select * from F_category");
	        // if page size is greater than zero then apply pagination
	        if (pageSize > 0) {
	            // Calculate start record index
	            pageNo = (pageNo - 1) * pageSize;
