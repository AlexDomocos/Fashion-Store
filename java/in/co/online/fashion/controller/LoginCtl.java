...
import in.co.online.fashion.model.UserModel;
...
public class LoginCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0) {
			...
			ServletUtility.forward(OFSView.LOGIN_VIEW, request, response);
			return;
		}
		if (session.getAttribute("user") != null) {
			ServletUtility.redirect(OFSView.WELCOME_CTL, request, response);
			return;
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SIGN_IN.equalsIgnoreCase(op)) {
			...
			ServletUtility.redirect(OFSView.USER_REGISTRATION_CTL, request, response);
		return;
		}
