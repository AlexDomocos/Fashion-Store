 ...
 */
@WebServlet(name = "FashionListCtl", urlPatterns = { "/ctl/FashionListCtl" })
public class FashionListCtl extends BaseCtl {
	...
	protected BaseBean populateBean(HttpServletRequest request) {
		log.debug("FashionListCtl populateBean method start");
		ClothesBean bean = new ClothesBean();
		bean.setName(DataUtility.getString(request.getParameter("name")));
		log.debug("FashionListCtl populateBean method end");
		return bean;
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		log.debug("FashionListCtl doGet method start");
		List list = null;
		int pageNo = 1;
		...
		try {
			...
			if(fatId>0) {
				ClothesBean cBean=new ClothesBean();
				cBean.setId(fatId);
				model.delete(cBean);
		...
		log.debug("FashionListCtl doGet method end");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		log.debug("FashionListCtl doPost method start");
		List list = null;
		...
		if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op) || OP_PREVIOUS.equalsIgnoreCase(op)) {
			...
			ServletUtility.redirect(OFSView.FASHION_LIST_CTL , request, response);
			return;
		...
		log.debug("FashionListCtl doPost method end");
	}

	@Override
	protected String getView() {
		// TODO Auto-generated method stub
		return OFSView.FASHION_LIST_VIEW;
	}
